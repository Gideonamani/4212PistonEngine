...
The immediate compatibility layer uses selector rules over the existing Blender object hierarchy. This is deliberate transitional metadata. Future CAD/Blender exports must write stable module and instance IDs as GLB extras; the web runtime will then select IDs directly instead of matching names. The contract also declares a deterministic gzip delivery file and its raw fallback, keeping transport, browser loading, and release validation bound to the same model identity.
