...
The V5 GLB remains a compatibility export: its hierarchy names are used only through contract selectors. The exporter is now prepared to emit stable `engine_id`, `module_id`, and `instance_id` extras, and the browser adapter will prefer those IDs as soon as a reviewed re-export is promoted. The present hash-bound Pages asset has not been silently replaced. M4 remains open pending applicable manual datums/bank angles, journal-to-bearing mapping, crank-throw indexing, primary-drive ratios, and whole-engine valve/gas evidence. These gaps bound M5; the detailed single-cylinder lesson remains the sole owner of its valve/gas assertions.
