...
Latest engineering result: the housing-clearance candidate passes all four listed moving-pair checks at 58 sampled poses. The separate native spring audit fails because the outer coil has zero axial pitch gap at the illustrative full lift. A spring seat/envelope correction is in progress; see [housing clearance review](housing-clearance-review.md). No candidate is promoted yet. Initial M3 publication checks run mechanism/transfer tests and reject a stale generated motion profile; the controlled CAD-edit propagation exercises remain open. The instructor reports that the emulated mobile view looks good; physical A16 testing remains pending.
